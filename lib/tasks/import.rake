  ...
  namespace :import do
    ...
    task :features => [:prepare, :environment] do
      ...
      File.open(txt_file) do |f|
        ...
        insert_data(f, GEONAMES_FEATURES_COL_NAME, Geonames::City, :title => "Features")
      end
    end
    ...
    %w[15000 5000 1000].each do |population|
      ...
      task "cities#{population}".to_sym => [:prepare, :environment] do
        ...
        File.open(txt_file) do |f|
          insert_data(f, GEONAMES_FEATURES_COL_NAME, Geonames::City, :title => "cities of #{population}")
        end
      end
    end
    ...
    task :admin1 => [:prepare, :environment] do
      ...
      File.open(txt_file) do |f|
        insert_data(f, GEONAMES_ADMINS_COL_NAME, Geonames::Admin1, :title => "Admin1 subdivisions") do |klass, attributes, col_value, idx|
    ...
    task :admin2 => [:prepare, :environment] do
      ...
      File.open(txt_file) do |f|
        insert_data(f, GEONAMES_ADMINS_COL_NAME, Geonames::Admin2, :title => "Admin2 subdivisions") do |klass, attributes, col_value, idx|
          col_value.gsub!('(general)', '')
          if idx == 0
